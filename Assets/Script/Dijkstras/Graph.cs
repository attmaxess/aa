...
namespace Dijkstras
{
    public class Graph : MonoBehaviour
    {
        ...
        /// </summary>
        [SerializeField]
        protected List<Hole> m_Nodes;

        /// <summary>
        /// Gets the nodes.
        /// </summary>
        /// <value>The nodes.</value>
        public virtual List<Hole> nodes
        {
            get
            {
                return m_Nodes;
        ...
        public virtual Path GetShortestPath(Hole start, Hole end)
        {
            ...
            if (start == end)
            {
                path.m_Nodes.Add(start);
                return path;
            }

            // The list of unvisited nodes
            List<Hole> unvisited = new List<Hole>();

            // Previous nodes in optimal path from source
            Dictionary<Hole, Hole> previous = new Dictionary<Hole, Hole>();

            // The calculated distances, set all to Infinity at start, except the start Node
            Dictionary<Hole, float> distances = new Dictionary<Hole, float>();
            ...
            while (unvisited.Count != 0)
            {
                ...
                {
                    Hole neighbor = current.connections[i];

                    // Getting the distance between the current node and the connection (neighbor)
                    float length = Vector3.Distance(current.HoleCenter(), neighbor.HoleCenter());
        ...
        [ContextMenu("GetAllHolesFromScene")]
        public void GetAllHolesFromScene()
        {
            Level level = GetComponentInParent<Level>();
            this.m_Nodes = level.GetComponentsInChildren<Hole>().ToList();
        }
    }
}
